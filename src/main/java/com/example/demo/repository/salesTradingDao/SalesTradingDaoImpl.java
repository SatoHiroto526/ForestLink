package com.example.demo.repository.salesTradingDao;
...
public class SalesTradingDaoImpl implements SalesTradingDao {
	...
	public Optional<Trading> purchaseTradingDetail(int trading_id) {
		...
		buf.append(" ON salestrading.purchaseenterprise_id = partnerenterprise.partnerenterprise_id ");
		buf.append(" INNER JOIN partneruser ");
		buf.append(" ON salestrading.purchaseuser_id = partneruser.partneruser_id ");
	...
	public Optional<Trading> salesTradingDetail(int trading_id) {
		...
		buf.append(" ON salestrading.salesenterprise_id = entryenterprise.enterprise_id ");
		buf.append(" INNER JOIN user ");
		buf.append(" ON salestrading.salesuser_id = user.user_id ");
		buf.append(" WHERE trading_id = ? ");
	...
	public int tradingInsert(Trading trading) {
		try {
			...
			buf.append(" INSERT INTO salestrading(product_id, orderday, purchaseday, ");
	...
	public int tradingUpdate(Trading trading) {
		try {
			...
			buf.append(" UPDATE salestrading SET product_id = ?, orderday = ?, purchaseday = ?, ");
			buf.append(" purchaseenterprise_id = ?, salesenterprise_id = ?, purchaseuser_id = ?, salesuser_id = ?, salesworktype = ?, count = ?, price = ? ");
			buf.append(" WHERE trading_id = ? ");
			check = jdbcTemplate.update(buf.toString(), trading.getProduct_id(), trading.getOrderday(), trading.getPurchaseday(), 
	...
	public int tradingDelete(int trading_id) {
		...
		buf.append(" DELETE FROM salestrading ");
		buf.append(" WHERE trading_id = ? ");
